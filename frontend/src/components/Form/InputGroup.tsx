    ...
    return (
        <div>
            <div className='relative w-full'>
                <input
                    className={`block py-2.5 w-full rounded-2xl appearance-none border bg-transparent focus:outline-none transition duration-300 text-zinc-800 focus:ring-0 ${
                        error
                            ? 'border-violet-600'
                            : 'border-zinc-300 focus:border-zinc-600'
                    } ${
                        attributes.name === 'password' ? 'pl-4 pr-10' : 'px-4'
                    } peer`}
                    placeholder=' '
                    {...attributes}
                    type={
                ...
                <label
                    ...
                    className={`absolute text-sm appearance-none bg-white duration-300 transform -translate-y-4 scale-75 top-1.5 z-10 origin-[0] px-2 peer-focus:px-2 ${
                        error
                            ? 'text-violet-600'
                            : 'text-zinc-800 peer-focus:text-zinc-600'
            ...
            {error && (
                <p className='mt-1 text-xs text-violet-600 italic'>{error}</p>
            )}
        </div>
    );
};
