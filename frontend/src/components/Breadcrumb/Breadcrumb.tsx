...
const Breadcrumb = ({ items, active }: BreadcrumbProps) => {
    return (
        <section className="bg-no-repeat bg-cover bg-left-top block mt-2 lg:mt-0 text-xs md:text-sm">
            <Container>
                {active ? (
                    <ul className="py-4">
                        <li className="inline-block leading-5">
                            <Link
                                to="/"
                                className="text-zinc-500 hover:text-violet-700 transition duration-150 ease-in-out"
                            >
                                Главная страница
                            </Link>
